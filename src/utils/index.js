...
export async function getStats(from, to) {
  ...
  const pipeline = [
    ...
    // group by month and category, summing expenses and income separately
    {
      $group: {
    ...
    // unwind the per-item array into documents (preserve months with no expenses)
    {
      $unwind: {
        path: "$totalExpenseByCategory",
        ...
        expenseByCategory: {
          $map: {
            ...
            in: {
              ...
              percentOfExpenses: {
                $cond: [
                  // check if totalExpenses > 0 to avoid division by zero and
                  // calculate percentage, round to 2 decimals, otherwise set to 0
                  { $gt: ["$totalExpenses", 0] },
                  {
                    $round: [
                      {
                        $multiply: [
                          { $divide: ["$$c.totalExpense", "$totalExpenses"] },
                          100,
                        ],
                      },
                      2,
